...
def fbdisconnect():
	if 'facebook_id' in login_session:
		...
		if facebook_id is None:
			response = make_response(json.dumps('Currently not logged in'), 401)
			response.headers['Content-Type'] = 'application/json'
			return response
	...
	if result['status'] == '200':
		...
		return "login_session not deleted"
...
def restaurants_list():
	if 'username' in login_session:
		user_restaurants = session.query(Restaurant).filter_by(user_id = login_session['user_id']).all()
		restaurants = session.query(Restaurant).filter_by(user_id = None).all()
		return render_template('restaurants_list.html', restaurants = restaurants, user_restaurants = user_restaurants)
	else: 
		restaurants = session.query(Restaurant).filter_by(user_id = None).all()
